...
const ItemList = ({items}) => {
    ...
    return (
        ...
        {items.map((item)=>(
           ...
           <div className="p-2 m-2 border-gray-200 text-left flex"
          key={item.card.info.id}
        >
          <div className="w-9/12">
            <span className='text-[#02060C] font-bold'>{item.card.info.name}</span>
            <div>
              <span> ₹{item.card.info.price / 100 || item.card.info.defaultPrice/100} <span className='text-[12px] font-  text-[#02060C] font-bold tracking-wide leading-3 '>🏷️40% OFF USE SWIGGYIT</span></span>
            </div>
